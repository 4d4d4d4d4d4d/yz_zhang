...
docs/specs/       # 功能拆分 spec（01~15）+ 追溯矩阵（16）+ 编排循环（17）
                  ...
                  #   国际化（55）Agent 交付闭环与产出审核（56）客户端契约覆盖（57）时区（58）合作体/团队/开发者界面（59）形状对齐闸门（60）编排接 agent 与经验回流（61）团队月度预算池（62）
...
web/              # Web 前端：React + Vite + PWA（34 tests，含管理后台/移动端约束/优惠页/纠纷面板/AI 助理面板与核验台/合作体与团队/开发者设置）
app/              # App：React Native / Expo（含纠纷答辩、视差发现流；CI 有解析闸门，仍未类型检查，见 DSPR-042）
