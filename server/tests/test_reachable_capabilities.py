...
MUST_BE_REACHABLE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "proposeChange": (
        ("web", "app"),
        "任务范围一变（「加了两个房间」），这是唯一能把它落地的路。"
        "没有它，双方只剩取消（按违约规则算补偿）或发起纠纷——"
        "**一件本该好商量的事，产品逼着他们走对抗路径**",
    ),
    "acceptChange": (
        ("web", "app"),
        "提案没人能接受，等于没有提案；接受时服务端会自动补/退托管差额",
    ),
    "changeOrders": (
        ("web", "app"),
        "**列表接口本身此前不存在**：对方拿不到 order_id，有按钮也点不了。"
        "只补按钮不补列表，按钮点不了",
    ),
    "defineMilestones": (
        ("web",),
        "分期只能在双签前定义，而两端都没人调用——**生产环境里每一份合约"
        "都只有一期**，网页上那张分期表的渲染条件永远不成立。"
        "App 侧未做：分期是发布方在成交前的设置动作，先在网页上做全",
    ),
    "checkin": (
        ("app",),
        "到场打卡带距离校验，没有入口就意味着**到场证据链从来没产生过一条**。"
        "只要 App：浏览器里的定位在上门场景里没有意义",
    ),
    "myApplications": (
        ("web", "app"),
    ...
    "exportEvidence": (
        ("web",),
        "纠纷当事人自己留底的唯一手段（平台的处理决定不是终局）。"
        "App 侧未做：导出文件在手机上的落点还没想清楚，不装作做了",
    ),
    "myInvitations": (
        ("web", "app"),
