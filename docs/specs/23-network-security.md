...
- **SEC-040** 事件总线跨副本。
  ✅ **V53 已实现，但没有用 Redis**：跨副本能力由**库里的发件箱** + drain job
  提供——任何副本都能补做任何副本发布的事件，`(event_id, handler)` 唯一约束
  保证恰好一次。少一个中间件、少一处可用性依赖，
  且「只执行一次」由数据库约束保证而非代码自觉。
  详见 [28-event-delivery.md](28-event-delivery.md)。
