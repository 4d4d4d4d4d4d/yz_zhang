...
- **SPEC-001 §3.1 / SPEC-005 面积模型只认能力"有无",不认"大小"(PPA 的 A 轴对 sizing 失效)** —— 🟡 **MAC/VAU/DSB/AVP 已修复**(SPEC-013 物理模型:面积随规模缩放 —— PE 数 / lanes / SRAM 字节 / vector_width+LUT;能量按 Horowitz ISSCC'14 per-op),5 个 compute 模块中 4 个已迁移,只剩 **DAGC** 按 SPEC-013 §5 跟进。原始 AVP-vector_width 不敏感的子发现亦已修复。以下为原始记录: 系统性实测(`test_area_model_sensitivity.py`):`total_area_um2` = Σ **active capability** 的固定 `area_cost_um2`,故面积对**能力开关**敏感(开 fp16 +18,000、关 double_buffer −9,000),但对**能力规模完全无感** —— MAC 阵列 32×32→64×64(4× PE)、VAU lanes 16→32、DSB buffer_kb→64、AVP vector_width→64,**四者面积 Δ=0**。真实硅面积随数据通路规模缩放,故这是结构性保真缺口:当前"加宽提吞吐"在面积上被建模为**零成本**(optimizer 实测 −48% drain 全部来自零面积代价的加宽),PPA 的 area 轴对 sizing 决策不可用。属 CLAUDE.md"面积系数是 pre-silicon 估计,待 Phase 5 标定"范畴,**已 filed 未静默改**。**v1.1/Phase 5 候选(SPEC-005 amendment)**:给每个 scaling config 项(array_rows×cols、lanes、buffer_kb、vector_width、read_throughput)定义面积项(如 area ∝ PE 数 / lane 数 / SRAM 字节),使 throughput↔area 权衡在 `sweep`/`optimize` 中可见;`test_area_model_sensitivity.py::TestScalingParamsDoNotChangeArea` 是 tripwire —— Phase 5 落地后它会失败,提示更新。
