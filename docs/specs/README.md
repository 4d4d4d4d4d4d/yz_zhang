...
- **保真度披露不一致:SPEC-005 compute 模块系数缺 `[calibration knob]` 就地标记** —— 审计(`docs/Fidelity-Audit.md`)发现:`[calibration knob]` provenance 标记在 SPEC-007(control)/ SPEC-010 / SPEC-011(DRAM)模块的 `AreaModel.notes` 里代码中显式存在,但 SPEC-005 compute 模块(MAC/DSB/VAU/AVP/DAGC)的 capability 面积/能量系数在代码里**没有**该标记,只有 SPEC-005 v1.1 amendment 文档声明"全部 calibration knob"。用户最常看的正是 compute 器件,却缺就地"这是占位值"提示。`test_fidelity_audit.py::TestCoefficientProvenanceDisclosure` 是 tripwire。**v1.1 候选**:给 compute 模块 capability 补 provenance(或 `Capability` 加 `provenance` 字段,SPEC-001 §3.1),使披露一致。总体保真度结论见 `docs/Fidelity-Audit.md`(机制真实/相对可信,绝对 PPA 系数为占位)。
- **SPEC-001 §3.1 / SPEC-005 面积模型只认能力"有无",不认"大小"(PPA 的 A 轴对 sizing 失效)** —— ✅ **5 个 compute 模块(MAC/VAU/DSB/AVP/DAGC）全部修复**(SPEC-013 物理模型:面积随规模缩放 —— PE 数 / lanes / SRAM 字节 / vector_width+LUT / unpack throughput;能量按 Horowitz ISSCC'14 per-op)。compute 数据通路已全部脱离占位常数;剩余占位在 control(SPEC-007)/ DRAM(SPEC-011),已标 `[calibration knob]`。原始 AVP-vector_width 不敏感的子发现亦已修复。以下为原始记录: 系统性实测(`test_area_model_sensitivity.py`):`total_area_um2` = Σ **active capability** 的固定 `area_cost_um2`,故面积对**能力开关**敏感(开 fp16 +18,000、关 double_buffer −9,000),但对**能力规模完全无感** —— MAC 阵列 32×32→64×64(4× PE)、VAU lanes 16→32、DSB buffer_kb→64、AVP vector_width→64,**四者面积 Δ=0**。真实硅面积随数据通路规模缩放,故这是结构性保真缺口:当前"加宽提吞吐"在面积上被建模为**零成本**(optimizer 实测 −48% drain 全部来自零面积代价的加宽),PPA 的 area 轴对 sizing 决策不可用。属 CLAUDE.md"面积系数是 pre-silicon 估计,待 Phase 5 标定"范畴,**已 filed 未静默改**。**v1.1/Phase 5 候选(SPEC-005 amendment)**:给每个 scaling config 项(array_rows×cols、lanes、buffer_kb、vector_width、read_throughput)定义面积项(如 area ∝ PE 数 / lane 数 / SRAM 字节),使 throughput↔area 权衡在 `sweep`/`optimize` 中可见;`test_area_model_sensitivity.py::TestScalingParamsDoNotChangeArea` 是 tripwire —— Phase 5 落地后它会失败,提示更新。
- **SPEC-005 estimate_latency 形状敏感 vs 运行时形状无关(MAC)** —— ✅ **已统一**(SPEC-013):MAC 的 `estimate_latency` 与运行时 `_compute_cycles` 现共用同一 systolic 模型 `_systolic_cycles(macs) = ceil(macs/PE) + (rows+cols)`。带 op_shape 的 SPEC-012 trace token 按 matmul 大小定时(32×32×32 → 96 拍,不再是常数 2);无 shape 的合成 token 退化到基础每-token 成本(故非 trace 测试零变动)。`test_mac_module.py::TestMACTimingModel` 锁定二者一致。实测 attention chip drain 4232→4326(avp 仍是瓶颈),模型误差仍 0.4%。**原始发现记录**:两个时序模型曾用不同公式,是 reconcile 静态估 vs 实测 gap 的来源之一。
