...
> 模块 spec：[63-app-store-readiness.md](63-app-store-readiness.md)
>
> ```
...
| Spec 功能点 | 实现 | 测试 |
|---|---|---|
| **APP-060 必填变成类型必填** | SDK 的 `createTask` 里 `ip_assignment` 改为**必需参数**。V77 加过一条「客户端确实发了这个字段」的闸门，但那条只看 Web；App 没有测试、CI 只做 `tsc`，而漏字段在 `Partial<Task>` 面前不是类型错误。**服务端的「必填」如果没有变成类型上的「必填」，那它只是一句口头约定** | `web/src/sdk-contract.test.ts`（`@ts-expect-error` 双向钉子；红验：把字段改回可选，`tsc` 立刻报「未使用的 ts-expect-error」） |
| **钉子要放在真的会执行的地方** | 这条钉子放在 `web/src/` 而不是 `packages/core/src/`——**只有前者真的过 `tsc`**，core 的测试只被 vitest 转译、不做类型检查。第一版就钉错了地方，红验时发现它根本不会红 | 同上 |
| **APP-061 账号注销** | 「我的」页；服务端的拦截理由（有余额、在途合约、纠纷）**原样显示**——否则用户看到的是一个莫名其妙的报错，而不是「你还有一笔托管中的资金」（CLI-064 同一条） | `app` 类型检查 + `STORE_CHECKLIST.md` 状态更新 |
| **APP-062/063 举报与拉黑** | 任务详情页；举报**带 `target_type`**：做成只发 reason 的按钮，运营侧收到的是一堆不知道在说什么的工单 | 同上 |
| **APP-064 协议与隐私政策** | 「我的」页，含版本号、待重新同意提示与 LAW-032 数据主体权利入口 | 同上 |

**这一批刻意没做的**：原计划是把合作体/团队/开发者三页搬到 App（UI-078）。
探针把优先级改了——**一个发不出任务的按钮和四条会被打回的必查项，
比三个新页面重要得多**。三条线的 App 页面继续挂着（APP-065）。
...
> 模块 spec：[59-coop-team-developer-ui.md](59-coop-team-developer-ui.md)
>
> 承 V82 的 CLI-065：合作体、团队、开放 API 三条线当时只补到 SDK，
> 网页上仍然没有页面。其中合作体是整个产品立意最重的一条，
> 到上一批为止只能用 curl 访问。

| Spec 功能点 | 实现 | 测试 |
|---|---|---|
| **判断：有后果的界面不能把风险写成小字** | 这三条线的每次点击后果都不对称——加入合作体的人以为是「加个群」，实际是投入可能血本无归、份额被后来者稀释、不可转让不可赎回。**风险不是免责声明，是决策所需的信息**；折叠进一行小字，等于知道它会影响决定却又不希望它影响决定 | — |
| **UI-070 风险揭示书在加入路径上** | 整篇 + 四条要点逐条展示，勾选后才能提交；**版本号取服务端的**——写死的话改版后用户签的是旧版而界面显示新版，签署记录与他看到的内容对不上 | `web/src/CoopTeamDeveloper.test.tsx::UI-070`（红验：把要点折叠掉即红） |
| **UI-071 份额是算出来的且会被稀释** | 原样显示服务端的 `basis`，不自己拼这句话：它是有后果的解释，应与算法同源 | `::UI-071/072/073` |
| **UI-072 确认按钮读 `can_confirm`** | 自己的贡献不能自己确认，客户端不重算这个条件 | 同上（红验：忽略 `can_confirm` 即红） |
| **UI-073 只分已实现收益** | 分配界面用 `realized_funds_cents` 并写明「只能分配已经实际收到的钱」——这是这个模式不变成「承诺收益的募集」的结构性条件之一 | 同上 |
| **UI-074 合规路径带理由与免责** | 逐项显示 `why`/`action`/状态 + `disclaimer`。只给清单不给理由，用户不知道哪些能省、哪些不能，最后要么全不做要么全找律师 | 同上 |
| **UI-075 团队：审批与开票读服务端判断** | `can_decide`（自己不能批自己）与 `invoice_block`（企业信息未核验）都来自服务端；超额支出显示「等待审批」而**不是失败**——申请已经建好了，报成失败用户会以为自己做错了什么 | `::UI-075`（两条：有权批 / 无权批） |
| **UI-076/077 开发者：一次性密钥与停用原因** | 明文密钥显示在需主动关闭的区域并原样显示服务端 `warning`；Webhook 自动停用显示 `disabled_reason`——**悄悄停掉比不停更坏**，集成方会以为平台还在发 | `::UI-076`、`::UI-077` |

**顺带修掉的两处自伤（都是前两批自己造的）**：

1. V82 给 `CompliancePath` 写的 TS 类型是错的（`items` 实际是
   `documents`/`registrations`/`notices`）。**类型对不上却没有任何东西会红**
   ——这正是 CLI-067「闸门只看路径不看形状」的代价，现在又多了一个实例。
2. V83 那条「客户端不许自己 `new Date`」的闸门写成了 `.ts` 测试并用了
   node 的 `fs`/`path`，而 web 的 tsconfig 不带 node 类型——**它会让 CI 的
   `tsc -p web` 直接报错**。我当时加完这个文件只跑了 vitest 没再跑 tsc。
   已移到 Python 扫描器那边（和其它跨语言扫描器放在一起），并补了红验。

> **新加的检查本身也要过一遍全部检查。** 闸门是代码，不是免检品。
