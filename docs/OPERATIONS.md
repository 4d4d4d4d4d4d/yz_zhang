...
| Verify | `/verify` | **核验台**（V82）：AI 助理没把握或判据没过的任务在这里由人来判；不能接的单子显示资格原因 |
| Ventures | `/ventures` | **早期合作体**（V84）：风险揭示书整篇展示后才能加入；贡献登记与确认计价、份额与稀释口径、只分已实现收益、合规路径清单 |
| Teams | `/teams` | **团队账户**（V84/V87）：成员**月度累计**额度、团队月度预算池与本月用量、支出申请与审批、企业信息与开票闸门 |
| Developer | `/developer` | **开发者设置**（V84）：API 密钥（明文只显示一次、可轮换吊销）与 Webhook（验签说明、投递记录、停用原因） |
| Admin | `/admin` | 管理后台：举报、工单、纠纷仲裁、对账、平台收入、审计日志 |

**AI 助理闭环**（V82 起网页上可走完）：任务详情页的「AI 助理」面板里
邀请助理 → 选人签约托管 → 触发执行 → 成功即自动交付，发布方验收放款；
置信度不足或判据没过时面板会显示**服务端给的闸门文案**，并给出
「申请人工核验」入口（升级触发的核验费由平台承担，主动核验由发布方承担）。
核验人在 `/verify` 接单、看产出与逐条判据、提交结论；结论为「通过 / 已修正」
时任务自动交付（已修正时交付物是**修正稿**）。
...
⚠️ **接对象存储时必须保住一条不变量**（V62）：文件名必须来自 CSPRNG，
**不得由文件内容派生**。`GET /files/{name}` 是匿名的能力 URL——CDN 直出时
读取端不可能鉴权，安全性完全依赖「名字不可猜」。改造前名字是
`sha256(内容)[:32]`，于是任何持有原图的人都能离线算出 URL，
端点变成一台「这张图在不在你这儿」的存在性预言机。
内容哈希只用于磁盘去重（`blobs/<sha256>` + 硬链接），不进 URL。
...
这套组合已被 **912 个测试**覆盖，其中 `test_concurrency_guards.py` 专门验证
...
⚠️ **时间一律是 UTC，且响应里带 `Z`**（V83 / TZ-060）。接入方、导出脚本、
BI 看板都按这个口径解析；**不要**把响应里的时间当本地时间。客户端提交时间
（例如任务截止时间）建议带偏移，不带偏移的服务端按 UTC 处理。
`app/core/timefmt.py` 是唯一出入口，有两道闸门（源码扫描 + 真实响应扫描）
盯着它不被绕过。

⚠️ **AI 助理的产出也过内容审核，但故障时的处理相反**（V81 / AGT-051、AGT-066）。
上传物故障时 fail open 放行进人审队列；**agent 产出故障时不放行**，run 直接
标 `escalated`，必须有人核验过才可能交付。理由是代价落在谁身上：上传物的
用途是自证，挡住它伤的是被侵害方；agent 产出的用途是换钱，放过它伤的是
发布方和平台自己。运维要知道的后果是：**内容安全供应商长时间故障时，
所有 agent 任务都会停在「待人工核验」**，核验单积压而不是交付积压。
先看 `/admin/vendors`，再决定是不是临时把 agent 目录停用（`is_active=false`）。
另外，被判 `reject` 的产出**正文不入库**，只留 labels——工单里用户问
「我的任务为什么失败了」时，`GET /tasks/{id}/agent-runs` 的 `error` 字段
有理由，但没有原文可看，这是有意的。
...
./deploy/restore.sh backups/db-20266822T030000Z.sql.gz
```
...
CI（`.github/workflows/ci.yml`）每次 push 自动跑：后端 850 测试、
前端 64 测试与构建、**App 安装与类型检查**、**alembic 迁移漂移检查**、
**真实 HTTP 主闭环冒烟**、**沙箱合规态闭环自检**、依赖扫描、恢复演练、
Postgres 双引擎迁移检查与部署验收。
...
- **裁决前先看被诉方答辩了没**（V61）。服务端会拦住答辩期内、被诉方未陈述
  的裁决（409 `response_window_open`），逾期才允许缺席裁决。看到这个 409
  **不是让你想办法绕过去**，是让你等：改造前网页上根本没有答辩入口，
  被诉方永远不可能陈述，每一份处理决定实际都是缺席作出的。
  `PLATFORM_DISPUTE_RESPONSE_HOURS` 调短会直接缩短对方的陈述机会，慎改。
...
定时任务编排的完整性、处置动作的一致性。这些有 912 个测试钉着。
