...
这套组合已被 **654 个测试**覆盖，其中 `test_concurrency_guards.py` 专门验证
...
./deploy/restore.sh backups/db-20265422T030000Z.sql.gz
```
...
CI（`.github/workflows/ci.yml`）每次 push 自动跑：后端 654 测试、
...
定时任务编排的完整性、处置动作的一致性。这些有 654 个测试钉着。
