...
这套组合已被 **681 个测试**覆盖，其中 `test_concurrency_guards.py` 专门验证
...
CI（`.github/workflows/ci.yml`）每次 push 自动跑：后端 681 测试、
前端 55 测试与构建、**App 安装与类型检查**、**alembic 迁移漂移检查**、
...
定时任务编排的完整性、处置动作的一致性。这些有 681 个测试钉着。
