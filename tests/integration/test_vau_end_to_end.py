"""End-to-end runtime test for the VAU module. SPEC-005 §6.3, SPEC-002 §6.

Producer -> VAU (in_a) -> Consumer. Happy path drains all tokens (INV-3); a
slow consumer makes VAU's output stall, attributed to the consumer (SPEC-002
§3.5).
"""
...
import npu_sim.modules  # noqa: F401  registers Producer / VAU / Consumer
...
@pytest.fixture
def chain_result():
    return elaborate_and_run(str(FIXTURES / "vau_chain.yaml"), max_cycles=500)


@pytest.fixture
def slow_result():
    return elaborate_and_run(str(FIXTURES / "vau_slow_consumer.yaml"), max_cycles=2000)


class TestVAUHappyPath:
    ...
    def test_all_tokens_flow_through_vau(self, chain_result):
        assert chain_result.tokens_delivered == {
            "prod.out→vau.in_a": 16,
            "vau.out→cons.in": 16,
        }
...
class TestVAUBackpressure:
    ...
    def test_vau_output_stalls_on_slow_consumer(self, slow_result):
        assert slow_result.per_module_stall_ps.get("vau", 0) > 0
    ...
    def test_all_tokens_delivered_despite_backpressure(self, slow_result):
        assert slow_result.tokens_delivered["vau.out→cons.in"] == 16
